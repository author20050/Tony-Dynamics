...
const frontendCandidates = [
  path.join(__dirname, 'index_1789055957712.html'),
  path.join(__dirname, 'attached_assets', 'index_1789055957712.html')
];
const publicFile = frontendCandidates.find(existsSync) || frontendCandidates[0];
...
function normalizeExternalSportsFeed(payload) {
  const incoming = Array.isArray(payload) ? payload : payload?.events;
  if (!Array.isArray(incoming)) return [];
  if (!incoming.some(event => event?.home_team && event?.away_team)) return incoming;
  return incoming.map(event => {
    const sportKey = String(event.sport_key || event.sport || 'football').toLowerCase();
    const sport = sportKey.includes('basket') ? 'basketball'
      : sportKey.includes('tennis') ? 'tennis'
      : sportKey.includes('volley') ? 'volleyball'
      : sportKey.includes('handball') ? 'handball'
      : sportKey.includes('cricket') ? 'cricket'
      : 'football';
    const bookmaker = event.bookmakers?.[0];
    const markets = (bookmaker?.markets || []).map(market => {
      const label = market.key === 'h2h' ? (sport === 'basketball' ? 'Moneyline' : 'Match result')
        : market.key === 'totals' ? 'Total points / goals'
        : market.key === 'spreads' ? 'Handicap / spread'
        : String(market.key || 'Market').replace(/[_-]+/g, ' ');
      return {
        id: market.key,
        label,
        options: (market.outcomes || []).map((outcome, index) => ({
          id: `${market.key}-${index}`,
          name: outcome.point === undefined ? outcome.name : `${outcome.name} ${outcome.point > 0 ? '+' : ''}${outcome.point}`,
          odds: Number(outcome.price)
        })).filter(option => Number.isFinite(option.odds))
      };
    }).filter(market => market.options.length >= 2);
    const startAt = event.commence_time || event.startAt;
    const live = Boolean(event.completed === false && startAt && Date.parse(startAt) <= Date.now());
    return {
      id: event.id,
      sport,
      league: event.sport_title || event.league || sportKey,
...
app.get('/api/sports-feed', async (_req, res, next) => {
  try {
    if (!process.env.SPORTS_API_URL) return res.json({ source: 'demo', events: [] });
    const endpoint = new URL(process.env.SPORTS_API_URL);
    const headers = { Accept: 'application/json' };
    if (process.env.SPORTS_API_KEY) {
      if (endpoint.hostname.includes('the-odds-api.com')) endpoint.searchParams.set('apiKey', process.env.SPORTS_API_KEY);
      else headers['x-api-key'] = process.env.SPORTS_API_KEY;
    }
    ...
    if (!response.ok) throw apiError(`Sports provider returned ${response.status}.`, 502);
    res.json({ source: 'provider', events: normalizeExternalSportsFeed(payload) });
  } catch (error) {
    next(error.status ? error : apiError('Sports provider is unavailable.', 502));
  }
});
...
app.post('/api/sports-bets', requireUser, async (req, res, next) => {
  try {
    ...
    const betType = req.body.betType === 'acca' ? 'acca' : 'single';
    ...
    if (betType === 'single' && selections.length !== 1) throw apiError('Singles bets require exactly one selection.');
    if (betType === 'acca' && selections.length < 2) throw apiError('Acca bets require at least two selections.');
    ...
    const result = await withTransaction(async client => {
      ...
      const bet = await client.query(`
        INSERT INTO sports_bets(user_id, stake, bet_type, total_odds, potential_return, selections, feed_reference)
        VALUES($1, $2, $3, $4, $5, $6, $7)
        RETURNING id, stake, bet_type, total_odds, potential_return, status, created_at
      `, [req.userId, stake, betType, totalOdds, potentialReturn, JSON.stringify(selections), req.body.feedReference || null]);
      await client.query(`
...
app.get('/api/sports-bets', requireUser, async (req, res, next) => {
  try {
    const bets = await query(`
      SELECT id, stake, bet_type, total_odds, potential_return, status, selections, created_at, settled_at
